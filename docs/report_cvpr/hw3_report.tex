...
\midrule
B-only & 512 & 5000 & 1000 & 10 & AdamW, wd $10^{-4}$ & $10^{-4}/10^{-5}$ & fixed & Action L1 + $10\cdot$KLD \\
A+B+C & 512 & 30000 & 1000 & 10 & AdamW, wd $10^{-4}$ & $10^{-4}/10^{-5}$ & 1k warmup, cosine to $10^{-5}$ & Action L1 + $10\cdot$KLD \\
\bottomrule
\end{tabular}
}
\end{table*}

联合训练前 1000 step warmup，随后在 30000 step 内 cosine decay。由于原始长训练 stdout 在空间清理时移除，但模型、配置和 10k/20k/30k checkpoint 均保留。图~\ref{fig:training} 故使用同一 D 抽样协议重新评价三个 checkpoint，属于 post-training checkpoint validation，不伪称为训练期在线验证。该曲线及完整 D 表格同时导出到名为 \texttt{act-checkpoint-reevaluation} 的 W\&B offline run；B 的 SDS 则是训练期间直接写入 W\&B。两者的来源在图注和运行配置中明确区分。

需要说明的是，题目希望两次训练使用相同网络和超参数以隔离数据来源。本实验保留了相同的 ACT 架构、batch、chunk、AdamW 初始学习率和 KL 权重，但最终 B-only 只训练 5000 step 且使用 fixed LR，A+B+C 训练 30000 step 且使用 warmup+cosine。因而最终模型比较同时包含训练环境、训练步数和调度器三个变化，不能作为“只增加 A/C 数据”的严格因果消融。后文把 17.60\% 称为最终配置差异；多环境数据的独立贡献仍需以相同步数、相同采样次数重新训练 B-only 才能确定。

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/task2_training_curve.png}
\caption{A+B+C cosine 检查点在 D 抽样评价上的误差趋势。该图用于说明 20k 到 30k 的动作误差收益已很小；最终表格使用完整 D shard 的两个模型对比。}
\label{fig:training}
\end{figure}

\subsection{评价协议与补充指标}

主结果使用同一脚本、同一 D 根目录和 batch size 64，对完整 D shard 的 92274 个样本计算教师强制 ACT loss。为了直接研究 Action Chunking，另取 B 和 D 各自按数据顺序排列的前 128 个 batch，即每个环境 8192 个样本；两个模型使用完全相同的样本顺序。受控实验调用 \texttt{predict\_action\_chunk} 得到 $\hat{\mathbf a}_{i,1:10}$，并定义分块位置误差
\[
...
\midrule
B only & 5k & fixed & 0.510606 & 0.508989 & 0.00016165 \\
\rowcolor{bestrow}
A+B+C & 30k & cosine & \textbf{0.419420} & \textbf{0.419402} & \textbf{0.00000176} \\
\bottomrule
\end{tabular}
\end{table*}
...
表~\ref{tab:offline} 和图~\ref{fig:offline} 支持两个结论。第一，最终 A+B+C 30k/cosine 配置相对最终 B-only 5k/fixed 配置将 Action L1 降低
\[
\frac{0.508989-0.419402}{0.508989}\times100\%=17.60\%,
\]
总损失降低 17.86\%。这证明联合配置在保留 D 数据上更准确，但不能把全部差异归因于训练环境多样性，因为训练预算和调度器也发生了变化。A+B+C 自身的同协议 checkpoint 评价显示，10k、20k、30k 的 D Action L1 分别为 0.42098、0.40852、0.40784；20k 到 30k 只再下降 0.00068（0.17\%），说明长训练后段已接近饱和，但它仍不能替代缺失的 B-only 30k 控制组。第二，该结果覆盖完整 D shard，而不是抽样 batch；因此它是本文最终 zero-shot 配置对比。受控 8192 样本子集上，B-only 与 A+B+C 的 clean D L1 分别为 0.5440 和 0.4467，下降 17.89\%，与完整 D 的 17.60\% 基本一致，说明后续分解没有依赖偶然抽样。
...
\midrule
B-only & 0.1856 & 0.5440 & 0.3584 & 193.10\% \\
A+B+C & 0.1208 & 0.4467 & \textbf{0.3259} & 269.90\% \\
\bottomrule
\end{tabular}
\end{table}

表~\ref{tab:crossenv} 中 A+B+C 的相对差看似更大，但这是因为其 B 基线低至 0.1208；判断泛化不能只看相对比值。绝对 B$\rightarrow$D 差距由 0.3584 降为 0.3259，缩小 9.1\%，同时 D 上绝对误差显著更低。对 128 个 clean-D batch 做配对统计，B-only 减 A+B+C 的平均 L1 差为 0.09698，95\% bootstrap CI 为 $[0.07270,0.12221]$，不跨 0，配对效应量 $d_z=0.67$。因此在这两个固定 checkpoint 之间，17.6\% 的差异不是由少数 batch 拉动，而是中等偏大的稳定效应；该统计结论约束的是 checkpoint 差异的稳定性，不消除训练配置混杂。

\subsection{Action Chunking 的分块位置鲁棒性}

ACT 的原始动机是一次建模动作序列，以减少逐步行为克隆中的误差累积并保持动作一致性~\cite{zhao2023act}。图~\ref{fig:chunk} 左侧显示，两模型在 D 上都呈轻微 U 形：中间位置最容易，首尾边界较难，但 A+B+C 在全部 10 个位置均低于 B-only。B-only 的 $E_1/E_{10}$ 为 0.5501/0.5577，A+B+C 为 0.4527/0.4666；对应 $R_{\mathrm{tail}}$ 分别为 1.000 和 1.017，末步/首步比为 1.014 和 1.031。即使在未见 D 中，10 步 chunk 的尾部误差也只比头部高约 0--1.7\%，没有出现随 $k$ 单调爆炸。
...
这一结果揭示了固定长度 chunk 的第一层鲁棒性：视觉编码只在 chunk 起点计算一次，十个 query 在同一个视觉上下文下联合产生，因此输出不会像逐步自回归动作预测那样把前一步预测作为下一步网络输入；视觉偏移造成的是整块共同偏差，而不是 chunk 内部的递归数值发散。图~\ref{fig:heatmap} 进一步证明这一点。模糊、噪声、相机平移和遮挡引起的颜色在横向 $k=1\ldots10$ 上大体均匀，误差从第一步就进入，并没有在第十步突然放大。A+B+C 对相机平移和遮挡的相对退化较大，但同一行内仍然平坦，说明主要问题是视觉表征整体错位，而非动作解码器的远期失稳。
...
但这并不等价于闭环稳定。当前配置会连续执行 10 步且没有 temporal ensemble；如果第一帧视觉判断错误，同一偏差会持续覆盖整个控制块，期间无法利用新观测纠正。离线结果能够证明“chunk 内预测误差没有随 horizon 爆炸”，不能证明“机器人状态偏离后仍能恢复”。固定分块的机制性结论应表述为：它提高短时动作一致性并把递归预测误差限制在块边界，但以降低 10 步内反应频率为代价。

\subsection{方向一致性、动态幅度与动作维度}

仅看平均 L1 会混合方向错误、幅度错误和过度平滑。图~\ref{fig:chunk} 右侧显示，A+B+C 的六维机械臂方向余弦相似度均值为 0.653，B-only 为 0.535，绝对提升 0.117，说明联合训练不仅缩小了动作数值，还更可靠地预测了运动方向。夹爪开闭符号准确率由 90.76\% 提高到 92.44\%。

两模型的相邻动作差分 L1 几乎相同（0.2325 对 0.2316），但预测/真实时序变化比由 B-only 的 0.425 提高到 A+B+C 的 0.585。两者都存在明显过平滑，但 A+B+C 保留了更多真实动作变化。由此可见，联合训练的收益不是简单把 chunk 变得更平，而是同时改善方向与动态幅度；固定 chunk 提供连续性，多环境数据则降低了“连续但错误”的风险。
...
维度 & B-only & A+B+C & 改进 \\
\midrule
$\Delta x$ & 0.5859 & 0.4503 & 23.15\% \\
$\Delta y$ & 0.5643 & 0.4350 & 22.91\% \\
$\Delta z$ & 0.5613 & 0.4350 & 22.49\% \\
$\Delta roll$ & 0.7217 & 0.6568 & 8.99\% \\
$\Delta pitch$ & 0.7014 & 0.6064 & 13.55\% \\
$\Delta yaw$ & 0.4780 & 0.3871 & 19.01\% \\
gripper & 0.1954 & 0.1561 & 20.11\% \\
\bottomrule
\end{tabular}
\end{table}

表~\ref{tab:dims} 表明平移与夹爪改善约 20--23\%，而 roll/pitch 仍是最难维度，误差最高且收益最小。视觉分布变化首先破坏物体和末端执行器的空间对应；旋转又比平移更依赖局部接触姿态，因此即使总 L1 改善，精细对准仍是最可能造成实际任务失败的部分。
...
\midrule
外观 & 0.5566 & 0.4739 & 14.85\% & 0.2382 & 0.1991 \\
损坏 & 0.6353 & 0.5227 & 17.72\% & 0.4461 & 0.3169 \\
空间 & 0.5955 & 0.5863 & 1.55\% & 0.3715 & 0.4637 \\
相机缺失 & 0.6738 & 0.6199 & 7.99\% & 0.4613 & 0.5206 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{外观变化。} 暗化、增亮、低对比度和暖色偏移下，A+B+C 平均 L1 为 0.4739，仍比 B-only 的 0.5566 低 14.85\%；其预测漂移也更小。训练环境多样性确实学到了部分光照和颜色不变性。

\textbf{图像损坏。} 模糊和噪声把两模型误差都明显推高，但 A+B+C 保持 17.72\% 的优势，且预测漂移由 0.4461 降至 0.3169。多环境训练对局部纹理退化的收益与 clean D 的 17.89\% 几乎一致，说明其改进不是只记住 D 风格，而是形成了更稳定的视觉动作映射。

\textbf{空间几何变化。} 相机平移与中心遮挡下，A+B+C 的绝对优势只剩 1.55\%，预测漂移反而高于 B-only。具体而言，相机平移时 A+B+C 相对自身 clean D 退化 32.05\%，中心遮挡退化 30.46\%；B-only 分别为 10.15\% 和 8.79\%。这不是 A+B+C 的绝对误差更差，而是其 clean 基线很低、对固定双相机几何的利用更强，一旦像素位置与训练几何不一致，原有优势几乎消失。近期关于相机条件化策略的研究也发现，未显式输入相机外参的 ACT 等 RGB 策略会从静态背景推断视角，换视角后这种捷径会失效~\cite{jiang2025camera}；本实验的空间扰动结果与该机制一致。
...
移除静态相机时，B-only 从 0.5440 增至 0.5651，仅退化 3.88\%；A+B+C 从 0.4467 增至 0.5666，退化 26.84\%，两模型绝对误差几乎相同。这说明 A+B+C 的 clean-D 优势大量来自更有效地融合全局场景上下文，但这种优势依赖静态相机存在。静态相机缺失也是 A+B+C 唯一出现较明显尾部放大的条件，$R_{\mathrm{tail}}=1.096$、末步/首步比为 1.123，说明全局上下文对较远期动作尤其重要。移除腕部相机的影响更严重：B-only 达到 0.7825（$+43.84\%$），A+B+C 达到 0.6733（$+50.74\%$）。结合腕部 B$\rightarrow$D 像素分布差异约为静态视角 2.5 倍，可以判断腕部相机同时是最重要的近场控制来源和最强的 domain-shift 通道。
...
由此得到一个更严格的鲁棒性结论：\textbf{当前 10 步 ACT 对外观与局部图像损坏具有可量化的短时鲁棒性，但对相机几何偏移没有同等鲁棒性；多环境训练改善了动作表示，却强化了对固定双相机布局的依赖。} 若继续提升，应优先加入相机位姿随机化或显式外参编码，并把执行长度与预测长度解耦，例如保留 10 步预测但每 1--2 步重新查询，再用 temporal ensemble 融合重叠 chunk。这样才能在维持平滑性的同时恢复对新观测的反应能力。
...
任务二中，最终 A+B+C 30k/cosine 配置在完整 D shard 上比 B-only 5k/fixed 配置的 Action L1 低 17.60\%，配对子集差值的 95\% bootstrap CI 不跨 0；由于训练步数和调度器不一致，该数字不能解释为纯粹的多环境数据因果增益。10 步分块的尾部/头部误差比为 1.017，没有出现随 horizon 快速发散；机械臂方向余弦相似度由 0.535 提升到 0.653，预测时序变化比由 0.425 提升到 0.585。受控实验进一步限定了这一优势：外观和图像损坏下保留约 15--18\% 的绝对误差优势，但空间偏移下只剩 1.55\%，腕部相机缺失是最严重退化。Action Chunking 的主要收益是短时动作一致性，而不是自动获得相机几何不变性；在当前无 temporal ensemble 的 10 步执行设置中，错误视觉条件会一致地影响整个 chunk。闭环成功率仍因仿真资源缺失而未测得，本文没有用离线指标替代它。
